...
def build_abaqus_mesh_model(payload, job_dir):
    ...
    def inject_end_coupling_keyword_fallback(inp_path, boundary_record):
        ...
        block_lines = [
            ...
            "** Generated by abaqus_runner.py after writeInput().",
        ]
        for spec in solid_end_face_specs:
            ...
            if left_labels:
                ...
                block_lines.append("*Nset, nset={0}, instance={1}".format(left_set, instance_name))
                block_lines.extend(format_keyword_labels(left_labels))
            ...
            if right_labels:
                ...
                block_lines.append("*Nset, nset={0}, instance={1}".format(right_set, instance_name))
                block_lines.extend(format_keyword_labels(right_labels))
        ...
        block_lines.append("*Surface, type=NODE, name={0}".format(fallback["left_node_surface"]))
        block_lines.extend(left_surface_lines)
        block_lines.append("*Surface, type=NODE, name={0}".format(fallback["right_node_surface"]))
        block_lines.extend(right_surface_lines)
        block_lines.extend(
            [
                ...
                "*Kinematic",
                "1, 6",
                "** End SCLAS Abaqus 2019 end-coupling keyword fallback",
                "**",
            ]
        )
        ...
        try:
            ...
            fallback["line_count"] = len(block_lines)
        except Exception as exc:
            fallback["status"] = "failed"
