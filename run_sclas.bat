...
if exist "%VENV_PYTHON%" (
    ...
    if not errorlevel 1 (
        "%VENV_PYTHON%" "%PROJECT_DIR%code\sclas_remote_gui.py"
        goto :done
    )
    echo WARNING: Existing .venv Python could not start. Trying system Python instead.
)

where py >nul 2>nul
if not errorlevel 1 (
    py -3 "%PROJECT_DIR%code\sclas_remote_gui.py"
    goto :done
)
